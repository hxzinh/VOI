...
struct IT {
    ...
    void propagate(int id, int l, int r) {
        ...
        if (k != 0 || d != 0) {
            ...
            t[2 * id].valK += k;
            t[2 * id + 1].valK += kRight;
            t[2 * id].valD += d;
            t[2 * id + 1].valD += d;
            t[2 * id].sum += arithSum(k, d, m - l + 1);
            t[2 * id + 1].sum += arithSum(kRight, d, r - m);
    ...
    void update(int id, int l, int r, int i, int j, int k, int d) {
        ...
        propagate(id, l, r);
        int m = (l + r) / 2;
        update(2 * id, l, m, i, j, k, d);
        update(2 * id + 1, m + 1, r, i, j, k + max(0, m - max(i, l) + 1) * d, d);
    ...
    void update(int l, int r, int k, int d) { update(1, 1, n, l, r, k, d); }
...
int main() {
    ...
    for (int x = 1; x <= maxA; ++x) pos[x].push_back(0);
    for (int i = 1; i <= n; ++i) pos[a[i]].push_back(i);
    for (int x = 1; x <= maxA; ++x) pos[x].push_back(n + 1);
    ...
    for (int x = 1; x <= maxA; ++x) {
        ...
        for (int i = 1; i < pos[x].size(); ++i) {
            ...
            ans += t.query(max(1, a - 1), b - 1);  // possible bug: 1) forget taking max with 1; 2) max(1,
                                                   // b-1)
