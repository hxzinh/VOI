...
int undo(int x){
    for(int i = t; i >= 1; i--){
        if(a[i].l == a[i].en) continue;
        if(a[i].en <= x && x <= a[i].dist + a[i].en - 1) x = a[i].l + (x - a[i].en);
        else if(x < a[i].en && x >= a[i].l) x += a[i].dist;
        else if(a[i].en <= x && x <= a[i].l + a[i].dist - 1) x -= a[i].dist;
    }
    return x;
}
void full(){
    for(int i = 1; i <= t; i++){
        cin >> a[i].l >> a[i].dist >> a[i].en;
    }
    for(int i = 1; i <= k; i++) cout << undo(i) << " ";
}
int main()
{
    FastIO
