...
void sub1(){
    ...
    ok[1] = true;
    for(int i = 1; i <= R; i++) if(ok[i]) {
        if(i + a <= R) ok[i + a] = true;
        if(i + b <= R) ok[i + b] = true;
        if(i + c <= R) ok[i + c] = true;
        if(i >= L && i <= R) res += i;
    }

    cout << res.toString();
}
Bignum sub4(Bignum Leftt, Bignum Rightt){
    Bignum sumR = Rightt * (Rightt + 1) / 2;
    Bignum sumL = (Leftt - 1) * Leftt / 2;
    Bignum res = sumR - sumL;
    return res;
}
Bignum sub5(Bignum Leftt, Bignum Rightt){
    Bignum B(b), C(c);
    bool ok = false;
    B = B % 2; C = c % 2;

    if(B == Bignum("1") || C == Bignum("1")) ok = true;

    if(ok){
        Bignum newL = Leftt + 1;
        if(newL < Leftt) return sub4(Leftt, Rightt);

        cout << newL.toString() << " ";
        Bignum cntR = newL / 2, cntL = (Leftt - 1) / 2;
        Bignum sumR = cntR * cntR, sumL = cntL * cntL;
        Bignum res = sumR - sumL + sub4(newL, Rightt);
        return res;
    }
    Bignum cntR = (R + 1) / 2, cntL = (L - 1) / 2;
    Bignum sumR = cntR * cntR, sumL = cntL * cntL;
    Bignum res = sumR - sumL;
    return res;
}
int32_t main()
{
    ...
    Bignum Leftt(L), Rightt(R);

    if(R <= 10000000) sub1();
    else if(a == 1 || b == 1 || c == 1) cout << sub4(Leftt, Rightt).toString();
    else if(a == 2 || b == 2 || c == 2) cout << sub5(Leftt, Rightt).toString();
    else cout << 1;

    cout << '\n';
    cout << sub4(Bignum(11), Bignum(50)).toString();
    return 0;
}
